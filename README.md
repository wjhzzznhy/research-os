...
| **Agent Studio 管理台** | 支持管理员登录、智能体模板管理、Dify 风格工作流编排、节点级提示词/工具/知识库配置与前端运行测试（新增） |
| **模型服务配置** | 管理台支持配置兼容 OpenAI API 的 `API Key`、`Base URL`、默认模型，并可热更新后端 LLM 缓存（新增） |
...
│   │   ├── src/
│   │   │   ├── app/                  # App Router 页面
│   │   │   │   ├── (admin)/          # Agent Studio 管理台与智能体编排页面（新增）
│   │   │   │   ├── (large)/          # 落地页
│   │   │   │   ├── (main)/           # 主功能区
│   │   │   │   │   ├── (pages)/
│   │   │   │   │   │   ├── code/     # 代码生成
...
│   │   │   │   ├── services/         # 业务服务层
│   │   │   │   │   ├── agent_template_service.py # 智能体模板持久化与工作流图转换服务（新增）
│   │   │   │   │   └── icon/         # 图标嵌入与检索服务
│   │   │   │   └── workflows/        # 预置工作流 (8 种)
│   │   │   ├── core/                 # 核心配置 (DB, Celery, Config, Logger)
│   │   │   ├── domain/               # 领域层
│   │   │   │   ├── models/           # SQLModel 数据模型
│   │   │   │   │   ├── agent_template.py # 智能体模板模型（新增）
│   │   │   │   └── schemas/          # Pydantic 请求/响应模式
│   │   │   ├── prompts/              # Prompt 模板 (中/英)
│   │   │   ├── qa/                   # QA 图 (LangGraph RAG)
│   │   │   ├── repositories/         # 数据访问层
│   │   │   │   ├── agent_template_repository.py # 智能体模板仓储（新增）
│   │   │   ├── server/               # API 路由 & 中间件
...
# 兼容 OpenAI API 的模型服务配置，可在 /admin 系统设置中继续修改（新增）
LLM_API_KEY=your_llm_api_key
LLM_BASE_URL=https://api.siliconflow.cn/v1
LLM_MODEL_DEFAULT=Qwen/Qwen2.5-7B-Instruct
```

> `docker-compose.yml` 中的 `${DB_USER}`、`${DB_PASSWORD}`、`${LLM_API_KEY}`、`${LLM_BASE_URL}`、`${LLM_MODEL_DEFAULT}` 等变量会从此文件读取（新增）。
...
- **科研门户**: http://localhost
- **Agent Studio 管理台**: http://localhost/admin，默认管理员账号 `admin / admin123`（新增）
- **API 文档**: http://localhost/api/v1/docs
- **RustFS 控制台**: http://localhost/console
- **Draw.io**: http://localhost/drawio

### 5. 验证 Agent Studio（新增）

1. 进入 `http://localhost/admin` 并登录管理台（新增）。
2. 在「系统设置」中填写模型 `API Key`、`Base URL` 和默认模型，保存后后端会热更新 LLM 缓存（新增）。
3. 在「智能体管理」中新建智能体，进入编排页配置 LLM 节点身份定义、模型和输出格式（新增）。
4. 点击编排页右侧「运行测试」，输入测试问题后可看到后端返回的输出、状态和 Session ID（新增）。
...
- **管理台配置**: `/admin` 系统设置保存后会调用 `/api/v1/system/config`，无需重启即可让后续 LLM 调用使用新配置（新增）
- **工作流编排**: 保存智能体模板会写入数据库并同步生成可执行工作流，重新打开管理台仍可继续编辑（新增）
...
| 模块 | 路径 | 说明 |
| :--- | :--- | :--- |
| System | `/system` | 系统状态与健康检查；支持 LLM API Key、Base URL、默认模型热更新（新增） |
| Projects | `/projects` | 项目 CRUD |
| Tasks | `/tasks` | 异步任务管理 |
| Files | `/files` | 文件上传与管理 |
| Skills | `/skills` | 技能注册与执行 |
| Agents | `/agents` | 智能体会话管理；`/agents/templates` 支持智能体模板 CRUD、管理台保存与删除（新增） |
| Memory | `/memory` | 多层记忆读写 |
| Workflows | `/workflows` | 工作流配置与执行；管理台保存智能体模板时会同步生成可执行工作流图纸（新增） |
...
| 方法 | 路径 | 说明 |
| :--- | :--- | :--- |
| `GET` | `/api/v1/system/config` | 读取当前 LLM API Key 掩码、Base URL 和默认模型（新增） |
| `PUT` | `/api/v1/system/config` | 更新 LLM API Key、Base URL、默认模型，并清空后端 LLM 缓存（新增） |
| `GET` | `/api/v1/agents/templates` | 获取已保存的智能体模板列表（新增） |
| `POST` | `/api/v1/agents/templates` | 新建智能体模板并生成关联工作流（新增） |
| `PUT` | `/api/v1/agents/templates/{template_id}` | 更新模板元数据、画布结构和关联工作流（新增） |
| `DELETE` | `/api/v1/agents/templates/{template_id}` | 删除智能体模板（新增） |
| `POST` | `/api/v1/agents/run` | 执行指定智能体或工作流，管理台运行测试使用该接口（新增） |
...
管理员可访问 `/admin` 进入 Agent Studio 管理台，默认管理员账号为 `admin / admin123`（新增）。

| 功能 | 说明 |
| :--- | :--- |
| 智能体模板管理 | 支持新建、编辑、复制、删除智能体模板；默认展示 6 个内置智能体，已保存模板会从后端加载并合并展示（新增） |
| 后端持久化 | 新建或保存智能体会写入 `agent_templates` 表，并同步维护一条 `workflows` 可执行图纸（新增） |
| Dify 风格编排 | 支持开始、LLM、工具、条件分支、回复、知识库节点的可视化拖拽和连线（新增） |
| 节点级配置 | LLM 节点支持身份定义/系统提示词、模型、温度、Token、输出格式；工具节点支持工具选择；知识库节点支持检索说明；条件节点支持分支说明；回复节点支持回复模板（新增） |
| 运行测试 | 编排页内置运行测试面板，可输入测试问题并直接展示后端 `/agents/run` 返回的 `final_output`、状态和 Session ID（新增） |
| 模型服务配置 | 系统设置页支持配置兼容 OpenAI API 的服务，如 SiliconFlow、DashScope 或自建网关（新增） |
...
```text
start / llm / tool / condition / reply / knowledge
```

保存到后端后会转换为 LangGraph 执行器使用的图结构：

```text
start / agent / router / end
```
...
| 前端节点 | 后端执行节点 | 说明 |
| :--- | :--- | :--- |
| `start` | `start` | 工作流入口（新增） |
| `llm` | `agent` | 使用节点级模型、身份定义、输出格式调用 LLM（新增） |
| `tool` | `agent` | 按节点配置挂载工具，如 `academic_search`、`local_knowledge`（新增） |
| `knowledge` | `agent` | 自动挂载本地知识库检索工具，并注入检索说明（新增） |
| `condition` | `router` | 根据上一节点路由决策和连线标签选择下一节点（新增） |
| `reply` | `end` | 工作流终点（新增） |
...
| `Artifact` | 生成产物 |
| `AgentSession` | 智能体会话 |
| `AgentTemplate` | Agent Studio 智能体模板，保存名称、角色、系统提示词、模型参数、技能配置、前端画布和关联工作流 ID（新增） |
...
`AgentTemplate` 存储管理台中的智能体元数据和前端画布结构；每次创建或更新模板时，后端会根据模板内容生成或更新一条 `Workflow` 记录（新增）。

```text
AgentTemplate.workflow          # 前端画布结构，用于管理台展示和继续编辑（新增）
AgentTemplate.workflow_id       # 指向自动生成的 Workflow（新增）
Workflow.graph_config           # LangGraph 可执行图纸（新增）
AgentSession.context_snapshot   # 保存执行历史与上下文快照（新增）
```
...
| `LLM_MODEL_DEFAULT` | `qwen-plus` | 管理台新建智能体与默认 LLM 调用使用的模型名，可填写如 `Qwen/Qwen2.5-7B-Instruct`（新增） |
| `OPENAI_API_KEY` | — | OpenAI API 密钥 |
| `OPENAI_BASE_URL` | `https://api.openai.com/v1` | OpenAI API 地址 |
| `GOOGLE_API_KEY` | — | Google API 密钥 |

> 管理台 `/admin` 的系统设置页可热更新 `LLM_API_KEY`、`LLM_BASE_URL`、`LLM_MODEL_DEFAULT`，更新后会清空 LLM 实例缓存，使后续模型调用立即使用新配置（新增）。
...
后端启动时会通过 SQLModel 自动创建缺失的业务表，`agent_templates` 会随 API 服务启动自动补齐，不会删除已有数据（新增）。
...
```bash
curl http://localhost/api/v1/agents/templates
curl http://localhost/api/v1/system/config

curl -X POST http://localhost/api/v1/agents/run \
  -H "Content-Type: application/json" \
  -d '{"input":"请用一句话回复：工作流是否打通？"}'
```

### 前端缓存刷新（新增）

如果浏览器提示「重新启动即可更新」或管理台仍显示旧版本，可以清理 Web 容器内 `.next` 缓存后重启前端（新增）：

```bash
docker compose exec web sh -lc "rm -rf /app/apps/web/.next/*"
